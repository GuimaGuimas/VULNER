...
public class EmpresaClienteDAO {
    ...
    public void insertEmpresaCliente(EmpresaCliente empresaCliente) throws SQLException {
        String sql = "INSERT INTO EmpresaCliente (razaoSocial, email, senha, cnpj, enderecoEnderecoId) VALUES" +
                "(?, ?, ?, ?, ?)";
        ...
        ps.setInt(5, empresaCliente.getEnderecoEnderecoId());
    ...
    public void updateEmpresaCliente(EmpresaCliente empresaCliente) throws SQLException {
        String sql = "UPDATE empresaCliente SET razaoSocial = ?, email = ?, senha = ?, cnpj = ?, enderecoEnderecoId = ? WHERE empresaClienteId = ?";
        PreparedStatement ps = empresaClienteConexao.prepareStatement(sql);
        ...
        ps.setString(4, empresaCliente.getCnpj());
        ps.setInt(5, empresaCliente.getEnderecoEnderecoId());
        ps.setInt(6, empresaCliente.getEmpresaClienteId());
        ps.execute();
        ps.close();
    }

    public ArrayList<EmpresaCliente> readEmpresaCliente() throws SQLException {
        String sql = "SELECT empresaClienteId, razaoSocial, email, cnpj, enderecoEnderecoId FROM empresaCliente";
        PreparedStatement ps = empresaClienteConexao.prepareStatement(sql);
